...
def test_assign_cdap_rank(people, model_settings):
    ...
    expected = pd.Series([1, 1, 1, 2, 2, 1, 3, 1, 2, 1, 3, 2, 1, 3, 2, 4, 1, 3, 4, 2], index=people.index)
...
def test_individual_utilities(people, model_settings):
    state = workflow.State.make_default(__file__)
    cdap_indiv_and_hhsize1 = state.filesystem.read_model_spec(file_name="cdap_indiv_and_hhsize1.csv")
    ...
    pdt.assert_frame_equal(individual_utils, expected, check_dtype=False, check_names=False)
...
def test_build_cdap_spec_hhsize2(people, model_settings):
    ...
    with chunk.chunk_log(state, "test_build_cdap_spec_hhsize2", base=True) as chunk_sizer:
        cdap.assign_cdap_rank(state, people, person_type_map)
        ...
        spec = cdap.build_cdap_spec(state, interaction_coefficients, hhsize=hhsize, cache=False)
...
def test_cdap_explicit_error_terms_parity(people, model_settings):
    ...
    large_people["household_id"] = large_people.groupby("household_id").cumcount() * 1000 + large_people["household_id"]
    ...
    cdap_indiv_spec = state_no_eet.filesystem.read_model_spec(file_name="cdap_indiv_and_hhsize1.csv")
    interaction_coefficients = pd.read_csv(
        state_no_eet.filesystem.get_config_file_path("cdap_interaction_coefficients.csv"),
        comment="#",
    )
    interaction_coefficients = cdap.preprocess_interaction_coefficients(interaction_coefficients)
    cdap_fixed_relative_proportions = pd.DataFrame({"activity": ["M", "N", "H"], "coefficient": [0.33, 0.33, 0.34]})
    ...
    state_no_eet.rng().add_channel("household_id", large_people.drop_duplicates("household_id").set_index("household_id"))
    ...
    state_eet.rng().add_channel("household_id", large_people.drop_duplicates("household_id").set_index("household_id"))
