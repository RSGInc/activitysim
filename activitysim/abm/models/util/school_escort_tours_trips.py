...
def replace_school_escort_trips_with_survey_trips(school_escort_trips):
    """
    In estimation, replace modeled school escort half-tours with the matching
    survey trips before assigning the temporary school_escort_trip_id used
    downstream. The modeled escort trips are only a scaffold for identifying
    which half-tours to replace, since survey trip counts and ordering may not
    match the trips generated by ActivitySim's escort trip creation logic.

    E.g. parent gets coffee on way to dropping off child at school. Or child 1
    ...
    if missing_cols:
        raise RuntimeError(
            "Survey trips table is missing columns needed for school escort trips: "
            f"{sorted(missing_cols)}"
        )

    escort_half_tours = school_escort_trips[["tour_id", "outbound"]].drop_duplicates()
    escort_half_tours_index = pd.MultiIndex.from_frame(escort_half_tours)
    survey_half_tours_index = pd.MultiIndex.from_frame(
        survey_trips[["tour_id", "outbound"]]
    )
    survey_school_escort_trips = survey_trips.loc[
        survey_half_tours_index.isin(escort_half_tours_index)
    ].copy()

    return survey_school_escort_trips
