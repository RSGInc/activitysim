...
class Settings(PydanticBase, extra="allow", validate_assignment=True):
    ...
    * ``"simple"``
        ...
        Slower but provided for backward compatibility / reproducibility
        with results generated by older ActivitySim versions.

    Note the default is `simple` not because simple is preferred, but because
    many integration tests are built looking for the results derived from the
    original `simple` channel.  New models should generally prefer the `fast`
    channel type, as it is faster.
    """
